...
public class CTMUtils {
    ...
    private static final ThreadLocal<TileOverrideIterator.IJK> ijkIterator = ThreadLocal.withInitial(CTMUtils::newIJKIterator);
    private static final ThreadLocal<TileOverrideIterator.Metadata> metadataIterator = ThreadLocal.withInitial(CTMUtils::newMetadataIterator);

    private static final ThreadLocal<BlockOrientation> renderBlockState = ThreadLocal.withInitial(BlockOrientation::new);

    private static final AtomicInteger texturePackChangeCounter = new AtomicInteger(0);
    private static final ThreadLocal<Integer> texturePackChangeCounterLocal = ThreadLocal.withInitial(() -> 0);
    ...
    private static int resetStateIfNecessary(int countLocal, int countGlobal) {
        if (countLocal < countGlobal) {
            renderBlockState.get().clear();
            ijkIterator.set(newIJKIterator());
            metadataIterator.set(newMetadataIterator());
            texturePackChangeCounterLocal.set(countGlobal);
        }
        return countGlobal;
    }
    ...
    private static IIcon getBlockIconWithoutLock(IIcon icon, Block block, IBlockAccess blockAccess,
        ...
        BlockOrientation state = renderBlockState.get();
        TileOverrideIterator.IJK ijkIterator = CTMUtils.ijkIterator.get();
        if (blockAccess != null && checkFace(face)) {
            state.setBlock(block, blockAccess, x, y, z);
            state.setFace(face);
            TileOverride lastOverride = ijkIterator.go(state, icon);
            if (lastOverride != null) {
                return skipDefaultRendering(block) ? RenderBlocksUtils.blankIcon : ijkIterator.getIcon();
            }
        }
    ...
    private static IIcon getBlockIconWithoutLock(IIcon icon, Block block, int face, int metadata) {
        BlockOrientation state = renderBlockState.get();
        TileOverrideIterator.Metadata metadataIterator = CTMUtils.metadataIterator.get();
        if (checkFace(face) && checkRenderType(block)) {
            state.setBlockMetadata(block, metadata, face);
            TileOverride lastOverride = metadataIterator.go(state, icon);
            if (lastOverride != null) {
                return metadataIterator.getIcon();
            }
        }
        return icon;
    }
    ...
    public static IIcon getBlockIcon(IIcon icon, Block block, IBlockAccess blockAccess,
                                     int x, int y, int z, int face) {
        int countLocal = resetStateIfNecessary(texturePackChangeCounterLocal.get(), texturePackChangeCounter.get());
        long stamp = lock.tryOptimisticRead();
        IIcon value = getBlockIconWithoutLock(icon, block, blockAccess, x, y, z, face);
        if (!lock.validate(stamp)) {
            stamp = lock.readLock();
            try {
                resetStateIfNecessary(countLocal, texturePackChangeCounter.get());
